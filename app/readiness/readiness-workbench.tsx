  ...
  return <>
    ...
    <section className="readiness-workflow">
      <article className="readiness-panel identity-panel">
        ...
        {loaded && vault ? <div className="identity-ready"><span>ACTIVE DEVICE IDENTITY</span><code>{vault.did}</code><div className="readiness-actions"><button className="button button-secondary" type="button" onClick={() => downloadVault(vault)}>Download encrypted backup</button><Link className="button button-secondary" href="/">Switch identity in Foundry</Link></div></div> : <form onSubmit={forgeIdentity} className="readiness-form"><label>Passphrase · minimum 12 characters<input name="passphrase" type="password" minLength={12} autoComplete="new-password" required /></label><label>Confirm passphrase<input name="confirmation" type="password" minLength={12} autoComplete="new-password" required /></label><button className="button button-primary" disabled={busy !== ''}>{busy === 'identity' ? 'Creating…' : 'Create DID + download backup'}</button><p>Already have a vault? Restore it from the main Foundry identity control.</p></form>}
