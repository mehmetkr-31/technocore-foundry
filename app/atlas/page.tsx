      ...
      <section className="atlas-ledger">
        ...
        {atlas.contributions.length ? <div className="atlas-list">{atlas.contributions.map((item, index) => {
          ...
          return <article className="atlas-node" key={item.resultId}>
            ...
            <div className="atlas-proof"><span>ISSUER <b>ACCEPTED</b></span><span>GITHUB <b className={item.evidenceGithub === 'verified' ? '' : 'muted'}>{item.evidenceGithub?.toUpperCase() ?? 'NOT CHECKED'}</b></span><span>CI <b className={item.evidenceCi === 'verified' ? '' : 'muted'}>{item.evidenceCi?.replace('_', ' ').toUpperCase() ?? 'NOT CHECKED'}</b></span><span>EXECUTION <b className={item.verificationReceiptCount ? '' : 'muted'}>{item.verificationReceiptCount} RECEIPT{item.verificationReceiptCount === 1 ? '' : 'S'}</b></span><span>REVIEWS <b className={item.reviewReceiptCount ? '' : 'muted'}>{item.reviewReceiptCount} STRUCTURED</b></span><span>PEERS <b className={item.attestationCount ? '' : 'muted'}>{item.attestationCount} EVIDENCE EDGE{item.attestationCount === 1 ? '' : 'S'}</b></span><span>FINAL TCR <b className={item.finalizedReceiptId ? '' : 'muted'}>{item.finalizedReceiptId ? 'BOUND' : 'PENDING'}</b></span></div>
