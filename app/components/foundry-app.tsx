...
type Dialog = 'forge' | 'restore' | 'mission' | 'claim' | 'result' | 'accept' | 'verify' | 'create-mission' | 'announce' | null;
...
export default function FoundryApp() {
  ...
  async function submitResult(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (commit && !repository) throw new Error('Add the GitHub repository that contains this commit.');
      const evidence = {
  ...
  async function publishAnnouncement(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const passphrase = String(new FormData(event.currentTarget).get('passphrase') ?? '');
      const receiptUrl = new URL(`/api/receipts/${selectedResult.id}`, window.location.origin).toString();
