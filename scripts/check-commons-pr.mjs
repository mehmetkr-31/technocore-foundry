...
const allowed = changes.filter(([, path]) => /^commons\/dossiers\/fds_[a-f0-9]{24}\.json$/.test(path));
if (changes.length !== 1 || allowed.length !== 1 || allowed[0][0] !== 'A') {
  throw new Error('A Commons admission PR must add exactly one new commons/dossiers/fds_<24hex>.json file and change nothing else.');
}

const staged = execFileSync('git', ['ls-tree', head, '--', allowed[0][1]], { cwd: repositoryRoot, encoding: 'utf8' }).trim();
...
console.log(JSON.stringify({ commonsPullRequest: 'ok', added: allowed[0][1] }));
