...
import { verifyTechnocoreExport } from './technocore-records.ts';
import { TCLK_DOMAIN, TCLK_MAX_FRAME_CHARACTERS, TCLK_PREFIX } from './tclk-contract.ts';

export const TCLK_TRANSCRIPT_MAX_BYTES = 256 * 1024;
export const TCLK_TRANSCRIPT_MAX_FRAMES = 128;

const PREFIX = TCLK_PREFIX;
const DOMAIN = TCLK_DOMAIN;
const DID = /^did:key:z6Mk[1-9A-HJ-NP-Za-km-z]{44}$/;
const HEX32 = /^0x[0-9a-f]{64}$/;
const HEX33 = /^0x[0-9a-f]{66}$/;
const AMOUNT = /^[1-9][0-9]*$/;
const ASSET = /^[A-Za-z0-9_-]{1,32}$/;
const RAIL = /^(?:[a-z0-9][a-z0-9._-]{0,63}|PaperRail)$/;
const NONCE = /^[0-9a-f]{8,64}$/;
const MAX_FRAME_CHARS = TCLK_MAX_FRAME_CHARACTERS;
const SECP256K1_P = (1n << 256n) - (1n << 32n) - 977n;
const RAIL_ALIASES = new Map([['paperrail', 'paper'], ['paper-rail', 'paper']]);
...
function validCompressedSecp256k1Point(value) {
  if (typeof value !== 'string' || !HEX33.test(value) || !['02', '03'].includes(value.slice(2, 4))) return false;
  const x = BigInt(`0x${value.slice(4)}`);
  if (x >= SECP256K1_P) return false;
  const rhs = (x * x % SECP256K1_P * x + 7n) % SECP256K1_P;
  return rhs === 0n || modularPower(rhs, (SECP256K1_P - 1n) / 2n, SECP256K1_P) === 1n;
}

function normalizedRail(value) {
  const lower = value.toLowerCase();
  return RAIL_ALIASES.get(lower) ?? lower;
}

function offeredRailIncludes(rails, selected) {
  if (rails.includes(selected)) return true;
  const target = normalizedRail(selected);
  return rails.some((rail) => normalizedRail(rail) === target);
}
...
async function validateFrame(frame) {
  ...
  if (type === 'offer') {
    if (!['payer', 'payee'].includes(frame.role) || !AMOUNT.test(frame.amount) || !ASSET.test(frame.asset) || !['hash', 'point'].includes(frame.lock) ||
      ...
      !NONCE.test(frame.nonce) || !HEX32.test(frame.id)) fail('SHAPE', 'frame', 'Offer terms are malformed.');
    ...
    if (frame.paymentKey !== undefined && !validCompressedSecp256k1Point(frame.paymentKey)) fail('SHAPE', 'frame', 'Offer paymentKey is not a valid compressed secp256k1 point.');
    ...
    if (!HEX32.test(frame.ref) || !HEX32.test(frame.contract) || !/^0x(?:[0-9a-f]{64}|[0-9a-f]{66})$/.test(frame.statement) || !NONCE.test(frame.nonce) ||
      (frame.paymentKey !== undefined && !validCompressedSecp256k1Point(frame.paymentKey))) fail('SHAPE', 'frame', 'Accept fields are malformed.');
  } else if (type === 'lock') {
    ...
    if (frame.presig !== undefined && (!exactKeys(frame.presig, ['nonce', 's']) || !validCompressedSecp256k1Point(frame.presig.nonce) || !/^0x(?:[0-9a-f]{2}){1,32}$/.test(frame.presig.s))) fail('SHAPE', 'frame', 'PTLC pre-signature reference is malformed.');
  } else if (type === 'reveal') {
    if (!HEX32.test(frame.contract) || !HEX32.test(frame.secret) || (frame.ref !== undefined && (typeof frame.ref !== 'string' || !frame.ref))) fail('SHAPE', 'frame', 'Reveal fields are malformed.');
  } else if (type === 'refund' || type === 'cancel') {
    if (!HEX32.test(frame.contract) || (type === 'refund' && frame.ref !== undefined && (typeof frame.ref !== 'string' || !frame.ref)) || (frame.reason !== undefined && (typeof frame.reason !== 'string' || !frame.reason))) fail('SHAPE', 'frame', `${type} fields are malformed.`);
  } else if (type === 'heartbeat') {
    if (!HEX32.test(frame.contract) || !NONCE.test(frame.nonce) || (frame.note !== undefined && (typeof frame.note !== 'string' || !frame.note))) fail('SHAPE', 'frame', 'Heartbeat fields are malformed.');
...
async function analyzeFrames(frames) {
  ...
  for (let position = 1; position < frames.length; position += 1) {
    ...
    if (frame.type === 'offer') { valid = false; message = 'A transcript may not open a second contract.'; }
      ...
      else if ((offer.lock === 'hash' && !HEX32.test(frame.statement)) || (offer.lock === 'point' && !validCompressedSecp256k1Point(frame.statement)) || (offer.lock === 'point' && !frame.paymentKey)) { valid = false; message = 'accept statement or required point-lock key is invalid.'; }
      else {
      ...
      if (status !== 'accepted' || frame.contract !== contract || frame.from !== payer || !offeredRailIncludes(offer.rails, frame.rail)) { valid = false; message = 'lock must follow accept, name this contract, come from payer, and use an offered rail.'; }
      else { status = 'locked'; rail = frame.rail; railRef = frame.ref; message = 'Payer lock announcement is ordered and rail is permitted.'; }
      ...
      else if (frame.ref !== undefined && frame.ref !== railRef) { valid = false; message = 'reveal names a different rail reference.'; }
      else if (offer.lock === 'point') { valid = false; message = 'Point-witness scalar validation is intentionally unavailable in this browser profile.'; }
      ...
      if (status !== 'locked' || frame.contract !== contract || frame.from !== payer || (frame.ref !== undefined && frame.ref !== railRef)) { valid = false; message = 'refund must follow lock, name this contract and rail reference, and come from payer.'; }
      ...
      if (!['claimed', 'refunded', 'cancelled'].includes(status) || frame.contract !== contract || ![payer, payee].includes(frame.from) || frame.outcome !== status || (frame.rail !== undefined && rail !== undefined && frame.rail !== rail) || (frame.ref !== undefined && railRef !== undefined && frame.ref !== railRef) || (status === 'cancelled' && (frame.rail !== undefined || frame.ref !== undefined))) { valid = false; message = 'receipt must match this terminal state, party, settlement rail, and rail reference.'; }
      else { terminalReceipt = true; message = 'Terminal acknowledgement matches the derived state.'; }
    } else if (frame.type === 'heartbeat') {
      if (!['accepted', 'locked'].includes(status) || frame.contract !== contract || ![payer, payee].includes(frame.from)) { valid = false; message = 'heartbeat must come from a contract party while accepted or locked.'; }
      else { message = 'Party heartbeat is state-neutral.'; }
    }
...
export async function inspectTclkTechnocoreExport(bytes, room) {
  let exportReport;
  try {
    exportReport = await verifyTechnocoreExport(bytes, room);
  } catch (cause) {
    fail(
      'TECHNOCORE_EXPORT',
      'transport',
      `Technocore JSONL export is invalid: ${cause instanceof Error ? cause.message : 'verification failed'}`,
    );
  }

  const selected = exportReport.records.filter(
    (record) => record.signatureState === 'valid' && record.text.startsWith(PREFIX),
  );
  if (!selected.length) {
    fail('NO_SIGNED_TCLK', 'transport', 'The export contains no valid DID-signed tclk1 records.');
  }

  const transcript = selected.map((record) => record.text).join('\n');
  const inspection = await inspectTclkTranscript(textBytes(transcript));
  const records = selected.map((record) => {
    const frame = parseStrictJson(record.text.slice(PREFIX.length));
    const frameDid = frame && typeof frame === 'object' && !Array.isArray(frame) && typeof frame.from === 'string'
      ? frame.from
      : null;
    return {
      seq: record.seq,
      recordDid: record.from,
      frameDid,
      authorMatches: frameDid === record.from,
      text: record.text,
    };
  });
  const authorsBound = records.every((record) => record.authorMatches);

  return {
    ...inspection,
    layers: { ...inspection.layers, transportDid: authorsBound ? 'valid' : 'invalid' },
    transport: {
      source: 'technocore_export',
      room: exportReport.room,
      exportSha256: exportReport.sha256,
      exportBytes: exportReport.bytes,
      totalRecords: exportReport.records.length,
      selectedRecords: records.length,
      ignoredRecords: exportReport.records.length - records.length,
      signatureCounts: exportReport.counts,
      authorsBound,
      records,
      signedFields: ['room', 'nonce', 'text'],
      sequenceMetadata: exportReport.sequenceMetadata,
      timestampMetadata: exportReport.timestampMetadata,
      generationMetadata: exportReport.generationMetadata,
      inclusionProof: 'not_cryptographically_established',
    },
    caveats: [
      authorsBound
        ? 'Every selected tclk1 frame is bound to its frame-level from DID by a valid Technocore record signature.'
        : 'At least one valid Technocore record was signed by a DID different from the frame-level from DID.',
      'Replay follows JSONL order; Technocore seq and ts are unsigned server metadata. X-Room-Generation is an unsigned response header absent from the JSONL body.',
      'A copied JSONL file is not a cryptographic proof that the Technocore server included these records.',
      ...inspection.caveats.slice(1),
    ],
  };
}
